...
always  @(posedge sclk or negedge s_rst_n) begin
        if(s_rst_n == 1'b0)
                ref_en  <=      1'b0;
        else if(state == ARBIT && ref_req == 1'b1 && wr_en == 0 && rd_en == 0) //仲裁第二个周期，用在读写请求后马上又出现一个刷新请求，当前读写使能为1，但还不能处理刷新
                ref_en  <=      1'b1;
        else
                ref_en  <=      1'b0;
end
...
always  @(posedge sclk or negedge s_rst_n) begin
        if(s_rst_n == 1'b0)
                wr_en   <=      1'b0;
        else if(state == ARBIT && ref_req == 1'b0 && wr_req == 1'b1 && rd_req == 1'b0)
                wr_en   <=      1'b1;
        else
                wr_en   <=      1'b0;
end
...
always  @(posedge sclk or negedge s_rst_n) begin
        if(s_rst_n == 1'b0)
                rd_en   <=      1'b0;
        else if(state == ARBIT && ref_req == 1'b0 && rd_req == 1'b1)
                rd_en   <=      1'b1;
        else 
                rd_en   <=      1'b0;        
end
